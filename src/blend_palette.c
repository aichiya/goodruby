...
void CopyPalette(u16 palOffset, u16 destOffset, u16 numEntries)
{
    ...
    {
        u16 srcindex = i + palOffset;
        u16 destindex = i + destOffset;
        gPlttBufferUnfaded[destindex] = gPlttBufferUnfaded[srcindex];
    }
}
...
void FullBlendPalette(u16 palOffset, u16 numEntries, u8 coeff, u16 blendColor)
{
    ...
    for (i = 0; i < numEntries; i++)
    {
        ...
        gPlttBufferUnfaded[index] = ((r + (((data2->r - r) * coeff) >> 4)) << 0)
