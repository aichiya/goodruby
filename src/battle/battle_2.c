...
void SwitchInClearSetData(void)
{
    ...
    if (gBattleMoves[gCurrentMove].effect == EFFECT_BATON_PASS)
    {
        ...
        gStatuses3[gActiveBattler] &= (STATUS3_LEECHSEED_BANK | STATUS3_LEECHSEED | STATUS3_ALWAYS_HITS | STATUS3_PERISH_SONG 
		    | STATUS3_ROOTED | STATUS3_MUDSPORT | STATUS3_WATERSPORT | STATUS3_AQUA_RING | STATUS3_POWER_TRICK);
...
void bc_aura_message(void)
{
    ...
    if (gBattleExecBuffer == 0)
    {
        ...
        {
            case SPECIES_ARTICUNO: // satk+1, sdef+1
            ...
            case SPECIES_REGISTEEL: // def+1, sdef+1
                ...
                PrepareStringBattle(446, 1);
                break;
            case SPECIES_GROUDON: // atk+2, def+2
                gBattleMons[1].statStages[STAT_STAGE_ATK] += 2;
                gBattleMons[1].statStages[STAT_STAGE_DEF] += 2;
                gActiveBattler = 1;
                PREPARE_STAT_BUFFER(gBattleTextBuff1, STAT_STAGE_ATK)
                PREPARE_STAT_BUFFER(gBattleTextBuff2, STAT_STAGE_DEF)
