    ...
    case 0:
        ...
        for (i = 0; i < 8; i++)
        {
            ...
            case EVO_PARTY_MEMBER:
                {
                    u16 wantedspecies = gEvolutionTable[species][i].param;
                    u8 j;
                    for (j = 0; j < 6; j++)
                        if (GetMonData(&gPlayerParty[j], MON_DATA_SPECIES2, 0) == wantedspecies)
                        {
                            targetSpecies = gEvolutionTable[species][i].targetSpecies;
                            break;
                        }
...
u8 GetMoveTutorMoves(struct Pokemon *mon, u16 *moves)
{
    ...
    for (i = 0; i < MAX_MOVE_TUTOR_MOVES; i++)
    {
        ...
        if (gLevelUpLearnsets[species][i] == 0xFFFFFFFF)
            break;
        ...
        if (moveLevel <= level)
        {
            ...
            if (j == 4)
            {
                for (k = 0; k < numMoves && moves[k] != (gLevelUpLearnsets[species][i] & 0xFFFF); k++)
                    ;
